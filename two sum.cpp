...
//  CODEFORCE's
//
//  Created by Harsh Anand on 20/01/2024.
//
...
void solve(void){
     ...
     sort(v.begin(), v.end());
    long long low=0, high=a-1;

     vector<pair<int , int>> ans;
     bool x=true;
     while (low<high) {
          if (v[low]+v[high]==tar) {
               ans.push_back(make_pair(v[low], v[high]));
               low++;
               high--;
               x=false;
          }else if (v[low]+v[high]>tar){
               high--;
          }else{
               low++;
          }
     }
     if(x)ans.push_back(make_pair(-1, -1));
